...
func TestFormatConfig_UnknownAgent(t *testing.T) {
	out := formatConfig("unknown", map[string]interface{}{})
	if out != "" {
		t.Fatalf("expected empty output for unsupported agent, got %q", out)
	}
}

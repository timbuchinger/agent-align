...
func TestSyncer_EndToEnd(t *testing.T) {
	...
	if len(result.Agents) != len(agents) {
		t.Fatalf("expected %d agents, got %d", len(agents), len(result.Agents))
	}

	// Check codex output is TOML-like
	codexOut, ok := result.Agents["codex"]
	if !ok {
	...
	copilotOut, ok := result.Agents["copilot"]
	if !ok {
